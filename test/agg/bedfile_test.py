...
def test_nonstandard_chrom():
    filename = op.join(testdir, "data", "test_non_standard_chrom.bed")
...
def test_table_created():
    check_table(op.join(testdir, "data", "gene_annotations.short.db"))
...
def test_gene_annotations():
    ...
    input_file = op.join(testdir, "data", "exon_unions_mm10.bed")
    f = tempfile.NamedTemporaryFile(delete=False)
...
def test_random_importance():
    ...
    runner = clt.CliRunner()
    input_file = op.join(testdir, "data", "25435_PM15-000877_SM-7QK6O.seg")
...
def test_no_chromosome_limit():
    ...
    runner = clt.CliRunner()
    input_file = op.join(testdir, "data", "geneAnnotationsExonsUnions.short.bed")
...
def test_float_importance():
    ...
    runner = clt.CliRunner()
    input_file = op.join(testdir, "data", "test_float_importance.bed")
