...
def test_bedfile_to_multivec():
    ...
    input_file = op.join(testdir, "data", "sample.multival.bed")
    chromsizes_file = op.join(testdir, "data", "sample.multival.chrom.sizes")
...
def test_load_multivec_tiles():
    op.join(testdir, "data", "sample.bed.multires.mv5")
    # TODO: Make assertions about result


def test_states_format_befile_to_multivec():
    runner = clt.CliRunner()
    input_file = op.join(testdir, "data", "states_format_input_testfile.bed.gz")
    rows_info_file = op.join(testdir, "data", "states_format_test_row_infos.txt")

    # Into a temp dir: without `--output-file` the tool writes beside its input,
    # leaving a multi-megabyte artifact in `test/data`.
    with tempfile.TemporaryDirectory() as tmp_dir:
        result = runner.invoke(
            ...
            [
                input_file,
                "--output-file",
                op.join(tmp_dir, "out.multires.mv5"),
                "--format",
                "states",
                "--row-infos-filename",
                rows_info_file,
...
def test_ignore_bedfile_headers():
    runner = clt.CliRunner()
    input_file = op.join(testdir, "data", "3_header_100_testfile.bed.gz")
    rows_info_file = op.join(testdir, "data", "3_header_100_row_infos.txt")

    # 15, matching `3_header_100_row_infos.txt`. The original passed 10 and
    # asserted nothing, so the resulting broadcast error went unnoticed.
    # As above: keep the generated multivec out of the committed data dir.
    with tempfile.TemporaryDirectory() as tmp_dir:
        result = runner.invoke(
            ccc.bedfile_to_multivec,
            [
...
def test_retain_lines():
    runner = clt.CliRunner()
    input_file = op.join(testdir, "data", "sample2.multival.bed")
    chromsizes_file = op.join(testdir, "data", "sample2.multival.chrom.sizes")
    row_infos_file = op.join(
        testdir, "data", "states_format_test_row_infos_v2.txt"
    )
...
def test_chr_boundaries_states():
    ...
    data_file = op.join(testdir, "data", "chrm_boundaries_test.multires.mv5")
