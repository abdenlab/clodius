...
import clodius.tiles.db_tiles as cdt
import clodius.tiles.hdf_tiles as cht
...
def test_clodius_aggregate_bedfile():
    input_file = op.join(
        testdir, "data", "geneAnnotationsExonsUnions.hg19.short.bed"
    )
...
def test_clodius_aggregate_bedgraph():
    input_file = op.join(testdir, "data", "cnvs_hw.tsv")
    assembly_file = op.join(testdir, "data", "test_cnvs_assembly")
    ...
    # input_file = op.join(testdir, 'data', 'cnvs_hw.tsv.gz')
    # result = runner.invoke(
    # cca.bedgraph,
    # [input_file,
...
def test_clodius_aggregate_bedpe():
    input_file = op.join(testdir, "data", "Rao_RepA_GM12878_Arrowhead.txt")
...
def test_clodius_aggregate_bedgraph1():
    input_file = op.join(testdir, "data", "dm3_values.tsv")
    ...
    input_file = op.join(testdir, "data", "test3chroms_values.tsv")
