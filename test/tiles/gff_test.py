...
def test_tileset_info():
    filename = op.join(testdir, "data", "GCA_002918705.1_ASM291870v1_genomic.gff.gz")
...
def test_indexed_tiles():
    filename = op.join(testdir, "data", "genomic.10k.gff.gz")
    index = op.join(testdir, "data", "genomic.10k.gff.gz.tbi")
